...
def import_csv():
    ...
    try:
        ...
        for row in csv_data:
            serial = row['numero_serial'].strip().upper()
            equipo = db.session.query(Equipo).filter_by(numero_serial=serial).first()
            ...
            if equipo:
                ...
                equipo.datacenter = row['datacenter']
                equipo.estado = row['estado']
                print(f"🔄 Actualizado: {serial}")
            else:
                ...
                db.session.add(nuevo_equipo)
                print(f"➕ Insertado: {serial}")

        equipos = db.session.query(Equipo).all()
        vistos = set()
        for equipo in equipos:
            if equipo.numero_serial in vistos:
                db.session.delete(equipo)
            else:
                vistos.add(equipo.numero_serial)
